...
class SiriProxyConnection < EventMachine::Connection
	...
	def initialize(options)
		...
		@db = EM::Synchrony::ConnectionPool.new(:size => 20) do
    ::Mysql2::EM::Client.new(:host => '127.0.0.1', :username => 'root',
                                    :password => 'root',
                                    :database => 'siriproxy',
                                    :reconnect => true)
    end
...
class SiriIPhoneConnection < SiriProxyConnection
	...
	def initialize(options)
		super
		self.proxy_4s = options[:proxy_4s]
		self.name = "iPhone"
	end
	...
	def post_init
      ...
      start_tls(:cert_chain_file => "cert/dev/server.passless.crt",:private_key_file => "cert/dev/server.passless.key", :verify_peer => false)
    else
      if self.proxy_4s
        start_tls(:cert_chain_file => "cert/4s/server.passless.crt",:private_key_file => "cert/4s/server.passless.key", :verify_peer => false)
      else
        start_tls(:cert_chain_file => "cert/host.crt",:private_key_file => "cert/host.key", :verify_peer => false)
      end
    end
...
class SiriProxy
	def initialize(pluginClasses=[], options = {})
		...
		options = {}
		options[:proxy_4s] = proxy_4s

		

		EM.synchrony do
			EM::start_server('0.0.0.0', port, SiriIPhoneConnection,options) { |conn|
